...
const initialEvents = [
  {
    ...
    time: "10:00",
    clientId: "client123", // Added clientId
  },
  {
    id: "2",
    ...
    time: "14:30",
    clientId: "client456", // Added clientId
  },
  {
    id: "3",
    date: new Date().toISOString().split("T")[0],
  ...
  },
  {
    ...
    color: "bg-indigo-500",
    message: "Initial meeting for Project Alpha",
    time: "09:00",
    clientId: "client123", // Added clientId
  },
];

// Add clientId as a prop
function ClientCalendarView({ clientId }) {
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  // Filter initialEvents based on clientId when the component mounts
  const [events, setEvents] = useState(
    initialEvents.filter(event => event.clientId === clientId)
  );
  ...
  // Memoized month calculations (no change here)
  ...
  const getDayEvents = (day) => {
    // This function will now automatically work with the filtered 'events' state
  ...
  const handleCreateOrUpdateEvent = () => {
    const eventData = {
      ...
      ...form,
      clientId: clientId, // **Crucially, add the clientId to new events**
    };
  ...
  // Render functions remain largely the same, as they operate on the 'events' state
  const renderHeader = () => (
          ...
          <Dialog.Panel className="bg-[#324250] p-6 rounded-2xl shadow-xl w-full max-w-md border border-[#3a4d5c]">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg transition ${form.title.trim()
